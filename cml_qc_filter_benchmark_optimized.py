...
def _rolling_bounds(n: int, window: int) -> tuple[np.ndarray, np.ndarray]:
    half_left = window // 2
    half_right = window - half_left
    idx = np.arange(n)
    return np.maximum(0, idx - half_left), np.minimum(n, idx + half_right)


def _rolling_var_1d(x: np.ndarray, window: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    lo, hi = _rolling_bounds(x.size, int(window))
    c = cnt[hi] - cnt[lo]
    good = c > 0
    var = np.full(x.size, np.nan, dtype=np.float32)
    if good.any():
        sum1 = s1[hi[good]] - s1[lo[good]]
        mean = sum1 / c[good]
        raw = (s2[hi[good]] - s2[lo[good]]) / c[good] - mean * mean
        var[good] = np.maximum(raw, 0.0).astype(np.float32)
    return var, good


def _rolling_std_1d(x: np.ndarray, window: int) -> np.ndarray:
    var, good = _rolling_var_1d(x, window)
    out = np.full(var.shape, np.nan, dtype=np.float32)
    out[good] = np.sqrt(var[good], dtype=np.float32)
    return out


def _rolling_max_minperiod1(x: np.ndarray, window: int) -> np.ndarray:
    # Vectorized centered rolling max for the small windows used in plateau detection.
    x = np.asarray(x, dtype=np.float32)
    n = x.size
    half_left = window // 2
    half_right = window - half_left
    shifted = np.full((window, n), np.nan, dtype=np.float32)
    for row, offset in enumerate(range(-half_left, half_right)):
        if offset < 0:
            shifted[row, :offset] = x[-offset:]
        elif offset > 0:
            shifted[row, offset:] = x[:-offset]
        else:
            shifted[row] = x
    all_nan = np.isnan(shifted).all(axis=0)
    filled = np.where(np.isnan(shifted), -np.inf, shifted)
    out = filled.max(axis=0).astype(np.float32, copy=False)
    out[all_nan] = np.nan
    return out
...
def _detect_plateaus_np(rsl: np.ndarray, time_span: int = 3, max_thld: float = -85,
    ...
    steady_var, steady_good = _rolling_var_1d(rsl, time_span)
    steady = steady_good & (steady_var < (std_thld * std_thld))
    bad = low & steady
    if pad > 1:
...
def _blackout_mask_1d(rsl: np.ndarray, rsl_threshold: float = -65,
    ...
    isn = np.isnan(rsl)
    if not isn.any():
        return np.zeros(rsl.shape, dtype=bool)
...
def _temporal_pass_np(trsl: np.ndarray, tspan: int, thld: float, perc: float) -> bool:
    var, good = _rolling_var_1d(trsl, tspan)
    denom = int(good.sum())
    if denom == 0:
        return False
    high_count = int((var[good] > (thld * thld)).sum())
    return bool((high_count / denom) <= perc)
...
def build_qc_dataset(ds: xr.Dataset, proc_line: str = "full", n_workers: int = 1,
                     link_batch: int = 64, max_links: int | None = None,
                     parallel: str = "threads") -> xr.Dataset:
    ...
    executor = None
    if n_workers > 1 and parallel != "none":
        executor_cls = ThreadPoolExecutor if parallel == "threads" else ProcessPoolExecutor
        executor = executor_cls(max_workers=n_workers)
    try:
    ...
    out.attrs["qc_parallel"] = parallel
...
def main() -> None:
    ...
    parser.add_argument("--parallel", choices=["threads", "processes", "none"], default="threads", help="Parallel backend. Threads avoid copying large time-series arrays and are usually fastest.")
    ...
    with xr.open_dataset(args.cml_nc) as ds:
        qc = build_qc_dataset(ds, proc_line=args.proc_line, n_workers=args.n_workers,
                              link_batch=args.link_batch, max_links=n_links, parallel=args.parallel)
